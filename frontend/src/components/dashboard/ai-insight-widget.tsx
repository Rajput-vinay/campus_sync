...
export function AiInsightWidget({ role }: Props) {
  ...
  const generateInsight = async () => {
    ...
    try {
      ...
      if (role === "admin") {
        mockResponse =
          "Analysis complete: Trade CSA has shown a 15% drop in attendance on Thursday. Additionally, the class average dropped by 2%. I recommend assigning the remedial reading material to the affected students.";
      } else if (role === "teacher") {
        mockResponse =
          "Observation: 3 students are missing from class today. I recommend assigning the remedial reading material to the affected students.";
      } else if (role === "student") {
        mockResponse =
          "Study Tip: Your grades are currently in the top 10% of your class. Keep up the good work!";
      } else {
