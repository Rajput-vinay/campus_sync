      ...
      <div className="flex w-max min-w-full flex-col">
        ...
        {timeSlots?.map((time) => (
          <div className="flex border-b last:border-b-0 min-h-27.5" key={time}>
            ...
            {DAYS.map((day) => {
              // Find the day data with case-insensitive matching
              ...
              return (
                ...
                >
                  {/* make sure you have subject and teacher objects with names */}
                  {/* handle both populated object and ID string for teacher */}
                  {period &&
                    ...
                    <div className="h-full w-full rounded-md border bg-card p-3 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-2 border-l-4 border-l-primary">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          >
                            {period.subject.code || (period as any).className || "—"}
                          </Badge>
                        </div>
                        <h4 className="font-semibold text-sm leading-tight text-primary line-clamp-2">
                          {period.subject.name}
                        </h4>
                        {(period as any).className && (
                          <p className="text-[10px] text-muted-foreground mt-1 font-medium">
                            Class: {(period as any).className}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground mt-auto pt-2 border-t border-dashed">
                        ...
                        <span
                          ...
                          title={period.teacher && typeof period.teacher === 'object' ? (period.teacher as any).name : 'You'}
                        >
                          {period.teacher && typeof period.teacher === 'object' ? (period.teacher as any).name : 'You'}
                        </span>
                      </div>
                    </div>
                  ) : (
